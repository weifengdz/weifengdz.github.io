...
function $_2(ID) {return document.getElementById(ID);}
function Ds(i) {$_2(i).style.display = '';}
function Dh(i) {$_2(i).style.display = 'none';}
function Df(i) {$_2(i).focus();}
var tID=0;
function Tab(ID) {
...
function Inner(i, s) {try {document.getElementById(i).innerHTML = s;}catch(e){}}
...
function Tb(d, t, p, c) {
	for(var i=1; i<=t; i++) {
		if(d == i) {$_2(p+'_t_'+i).className = c+'_2'; Ds(p+'_c_'+i);} else {$_2(p+'_t_'+i).className = c+'_1'; Dh(p+'_c_'+i);}
	}
}
...
document.onkeydown = function(e) {
	...
	if(k == 37) {
		...
		try{if($_2('search_tips').style.display != 'none' || $_2('search_tips').innerHTML != ''){SCTip(k);return false;}}catch(e){}
	} else if(k == 27) {
